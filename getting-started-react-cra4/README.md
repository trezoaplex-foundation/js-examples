...
2. **Install the Trezoaplex and the Trezoa SDKs.**
   ...
   ```sh
   npm install @trezoaplex-foundation/js-next @trezoa/web3.js
   ```
...
4. **Override Webpack 4 configurations.**
   ...
   <details>
     <summary>Why?</summary>
     If we skip this step, running `npm start` will fail, cotplaining that it can't resolve `.mjs` files provided by the Web3.js library. To fix this, we need to help Webpack resolve `.mjs` files by adding an extra rule.
   </details>
